  ...
  const getErrorMessage = (err: any) => {
    ...
    switch (code) {
      ...
      case 'auth/api-key-not-valid':
        return (
          <div className="flex flex-col gap-2">
            <p className="font-bold text-red-700">Invalid Firebase API Key</p>
            <p>The API Key provided in your environment variables is not valid. Please double-check your Render Dashboard settings.</p>
            <p className="text-xs">Ensure <code className="bg-red-100 px-1 rounded">VITE_FIREBASE_API_KEY</code> starts with <code className="bg-red-100 px-1 rounded">AIza</code> and has no spaces or quotes.</p>
          </div>
        );
      default:
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex items-center justify-center p-4 font-sans relative">
      ...
      <motion.div 
        ...
        className="bg-white p-8 rounded-3xl shadow-xl border border-stone-100 w-full max-w-md relative"
      >
        {onBack && (
          <button 
            onClick={onBack}
            className="absolute -top-12 left-0 flex items-center gap-2 text-stone-500 hover:text-emerald-600 transition-colors font-bold text-sm"
          >
            <ArrowLeft size={16} />
            Back to Home
          </button>
