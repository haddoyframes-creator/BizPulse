...
function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  ...
  // In production audit logic, we MUST throw the JSON string
...
export const recordBillingHistory = async (
  ...
  const path = `users/${userId}/billing_history`;
  try {
  ...
  if (userId) {
    ...
    try {
      ...
      if (path.startsWith('/api/inventory')) {
        const fullPath = `users/${userId}/inventory`;
        if (method === 'POST') {
          const data = JSON.parse(options.body as string);
          try {
            await addDoc(collection(db, 'users', userId, 'inventory'), data);
            return new Response(JSON.stringify({ success: true }));
          } catch (e) { handleFirestoreError(e, OperationType.CREATE, fullPath); }
        } else if (method === 'PUT') {
          ...
          try {
            ...
            return new Response(JSON.stringify({ success: true }));
          } catch (e) { handleFirestoreError(e, OperationType.UPDATE, `${fullPath}/${id}`); }
          ...
          try {
            ...
            return new Response(JSON.stringify({ success: true }));
          } catch (e) { handleFirestoreError(e, OperationType.DELETE, `${fullPath}/${id}`); }
        }
      } else if (path.startsWith('/api/transactions')) {
        const fullPath = `users/${userId}/transactions`;
        if (method === 'POST') {
          const data = JSON.parse(options.body as string);
          try {
          ...
          try {
            ...
            if (oldTxDoc.exists()) {
              ...
              if (oldTx.product_id) {
                ...
                await updateDoc(productRef, { stock: increment(stockRevert) });
              }
            }
        ...
        const fullPath = `users/${userId}/customers`;
        if (method === 'POST') {
          const data = JSON.parse(options.body as string);
          delete data.id;
          try {
        ...
        if (method === 'POST') {
          ...
          try {
            ...
            return new Response(JSON.stringify({ success: true }));
          } catch (e) { handleFirestoreError(e, OperationType.WRITE, `users/${userId}/business_info/info`); }
        }
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'inventory' | 'transactions' | 'customers' | 'performance' | 'reports' | 'tax' | 'subscription' | 'settings' | 'admin' | 'ai-advisor' | 'landing'>('dashboard');

  // Simple fragment-based routing for "no hinderance" navigation
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash.replace('#', '') as any;
      const validTabs = ['dashboard', 'inventory', 'transactions', 'customers', 'performance', 'reports', 'tax', 'subscription', 'settings', 'admin', 'ai-advisor', 'landing'];
  ...
  useEffect(() => {
    ...
    }, 60000); // Refresh every minute for the countdown
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        ...
        // Listen to User
        unsubUser = onSnapshot(doc(db, 'users', userId), async (docSnap) => {
          if (docSnap.exists()) {
  ...
  if (!isConfigured && import.meta.env.PROD) {
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-3xl p-10 shadow-xl border border-stone-100">
          <div className="w-20 h-20 bg-amber-50 text-amber-600 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-sm">
            <AlertCircle size={40} />
          </div>
          <h1 className="text-2xl font-bold text-stone-900 text-center mb-4">Configuration Required</h1>
          <p className="text-stone-600 text-center mb-8 leading-relaxed">
            Your application is live on Render, but it hasn't been connected to Firebase. 
            You must add the required environment variables to your Render Dashboard.
          </p>
          <div className="bg-stone-50 rounded-2xl p-6 mb-8 space-y-3">
            <p className="text-xs font-bold text-stone-400 uppercase tracking-widest">Missing Variables:</p>
            <ul className="text-sm text-stone-700 space-y-2">
              <li className="flex items-center gap-2"><code className="bg-stone-200 px-1.5 py-0.5 rounded text-[11px]">VITE_FIREBASE_API_KEY</code></li>
              <li className="flex items-center gap-2"><code className="bg-stone-200 px-1.5 py-0.5 rounded text-[11px]">VITE_FIREBASE_AUTH_DOMAIN</code></li>
              <li className="flex items-center gap-2"><code className="bg-stone-200 px-1.5 py-0.5 rounded text-[11px]">VITE_FIREBASE_PROJECT_ID</code></li>
              <li className="flex items-center gap-2"><code className="bg-stone-200 px-1.5 py-0.5 rounded text-[11px]">VITE_FIREBASE_APP_ID</code></li>
            </ul>
          </div>
          <p className="text-xs text-stone-400 text-center italic">
            Check the <span className="font-mono">README_DEPLOY.md</span> file for step-by-step instructions.
          </p>
        </div>
      </div>
    );
  ...
  if (!token) {
    if (showAuth) {
      return <Login onLogin={setToken} onBack={() => setShowAuth(false)} />;
    }
    ...
    switch (activeTab) {
      ...
      case 'performance': 
        if (tier !== 'free' || isTrialActive) {
          return <PerformanceReports inventory={inventory} transactions={transactions} customers={customers} />;
        }
        return <SubscriptionGate 
          title="Performance Analytics" 
          description="Your 14-day free trial has ended. Upgrade to a paid plan to keep accessing advanced performance insights and business analytics." 
          requiredTier="basic"
          onUpgrade={() => changeTab('subscription')}
        />;
      case 'reports': 
        if (tier === 'basic' || tier === 'pro' || isTrialActive) {
          return <CACAnnualReport user={user} transactions={transactions} business={business} />;
        }
        return <SubscriptionGate 
          title="CAC Annual Report" 
          description="Trial expired. Generate your provisional CAC Annual Return document with auto-calculated financials in a paid plan." 
          requiredTier="basic"
          onUpgrade={() => changeTab('subscription')}
        />;
      case 'tax': 
        ...
        return <SubscriptionGate 
          title="Tax Expert" 
          description="Trial expired. Estimate your annual taxes according to Nigerian tax laws and get filing guidance in a paid plan." 
          requiredTier="basic"
          onUpgrade={() => changeTab('subscription')}
        />;
      case 'ai-advisor':
        ...
        return <SubscriptionGate 
          ...
          description="Trial expired. Get personalized business insights and recommendations based on your transaction history in a paid plan." 
          requiredTier="basic"
          onUpgrade={() => changeTab('subscription')}
        />;
      case 'subscription': return <Subscription user={user} business={business} onUpdate={fetchData} />;
      case 'landing': return <LandingPage onGetStarted={() => changeTab('dashboard')} />;
      case 'settings': return <SettingsView user={user} business={business} onUpdate={fetchData} onNavigate={changeTab} />;
      default: return <Dashboard stats={stats} transactions={transactions} onNavigate={changeTab} />;
    }
  };
    ...
    <div className="flex h-screen bg-stone-50 text-stone-900 font-sans">
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto overflow-x-hidden">
          <NavItem icon={<LayoutDashboard size={20} />} label="Dashboard" active={activeTab === 'dashboard'} onClick={() => changeTab('dashboard')} collapsed={isSidebarCollapsed} />
          <NavItem icon={<BarChart2 size={20} />} label="Performance" active={activeTab === 'performance'} onClick={() => changeTab('performance')} collapsed={isSidebarCollapsed} />
          <NavItem icon={<Package size={20} />} label="Inventory" active={activeTab === 'inventory'} onClick={() => changeTab('inventory')} collapsed={isSidebarCollapsed} />
          <NavItem icon={<PlusCircle size={20} />} label="Transactions" active={activeTab === 'transactions'} onClick={() => changeTab('transactions')} collapsed={isSidebarCollapsed} />
          <NavItem icon={<Users size={20} />} label="Customers" active={activeTab === 'customers'} onClick={() => changeTab('customers')} collapsed={isSidebarCollapsed} />
          <NavItem icon={<FileText size={20} />} label="CAC Report" active={activeTab === 'reports'} onClick={() => changeTab('reports')} collapsed={isSidebarCollapsed} />
          <NavItem icon={<Scale size={20} />} label="Tax Expert" active={activeTab === 'tax'} onClick={() => changeTab('tax')} collapsed={isSidebarCollapsed} />
          <NavItem icon={<Brain size={20} />} label="AI Advisor" active={activeTab === 'ai-advisor'} onClick={() => changeTab('ai-advisor')} collapsed={isSidebarCollapsed} />
          <NavItem icon={<Zap size={20} />} label="Subscription" active={activeTab === 'subscription'} onClick={() => changeTab('subscription')} collapsed={isSidebarCollapsed} />
          <NavItem icon={<LayoutDashboard size={20} />} label="Landing Page" active={activeTab === 'landing'} onClick={() => changeTab('landing')} collapsed={isSidebarCollapsed} />
          {(user?.role === 'admin' || user?.email?.toLowerCase() === 'haddoyframes@gmail.com') && (
            <NavItem icon={<Shield size={20} />} label="Admin" active={activeTab === 'admin'} onClick={() => changeTab('admin')} collapsed={isSidebarCollapsed} />
          )}
        ...
        <div className={cn(
          ...
          {!isSidebarCollapsed && user?.subscription_tier === 'free' && (
                 ...
                 return (
                   <>
                     ...
                     <button 
                       ...
                       className="w-full py-2 bg-emerald-600 text-white text-[10px] font-bold rounded-lg shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all uppercase tracking-widest"
                     >
                       Upgrade Now
                     </button>
                   </>
...
function AdminDashboard({ user }: { user: User | null }) {
  ...
  const viewUserDetails = async (userId: string) => {
    ...
    try {
      ...
      handleFirestoreError(err, OperationType.GET, `users/${userId}/business_info/info`);
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <span className={cn(
                        ...
                        getTrialInfo(u.trial_started_at).active ? "text-emerald-600" : "text-stone-400"
                      )}>
                        {getTrialInfo(u.trial_started_at).active 
                          ? `${getTrialInfo(u.trial_started_at).days}d ${getTrialInfo(u.trial_started_at).hours}h left` 
                          : "Expired"}
                      </span>
                      <div className="flex gap-2">
                        <button 
                          onClick={() => handleUpdateUser(u.id, 'trial_started_at', new Date().toISOString())}
                          className="text-[8px] text-blue-600 hover:underline font-bold uppercase"
                        >
                          Reset
                        </button>
                        <button 
                          onClick={() => {
                            ...
                            handleUpdateUser(u.id, 'trial_started_at', expiredDate.toISOString());
                          }}
                          className="text-[8px] text-rose-600 hover:underline font-bold uppercase"
                        >
                          End
                        </button>
                      </div>
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full hover:bg-emerald-100 transition-colors uppercase tracking-wider"
            >
              View Analytics
            </button>
          </div>
    ...
    try {
      ...
      if (editingItem) {
        ...
        try {
          ...
          handleFirestoreError(error, OperationType.CREATE, `users/${userId}/inventory`);
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        // Record all items in parallel for efficiency
        const savePromises = finalItems.map(item => 
        ...
        if (failed) throw new Error("Failed to save some items. Please check your internet connection.");
      }
...
// --- Reports Component ---
function PerformanceReports({ inventory, transactions, customers }: { inventory: InventoryItem[], transactions: Transaction[], customers: Customer[] }) {
  const [reportType, setReportType] = useState<'products' | 'customers'>('products');
  ...
  const productPerformance = inventory.map(item => {
    const itemSales = filteredTransactions.filter(t => t.product_id === item.id && t.type === 'sale');
    const totalQty = itemSales.reduce((sum, t) => sum + (Number(t.quantity) || 0), 0);
    const totalRevenue = itemSales.reduce((sum, t) => sum + t.amount, 0);
    return {
      name: item.name,
      id: item.id,
  ...
  const customerPerformance = customers.map(customer => {
    const customerTx = filteredTransactions.filter(t => t.customer_id === customer.id && t.type === 'sale');
    const totalSpent = customerTx.reduce((sum, t) => sum + t.amount, 0);
    const favoriteProductId = customerTx.reduce((acc, t) => {
      if (!t.product_id) return acc;
      acc[t.product_id] = (acc[t.product_id] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    
    const topProductId = Object.entries(favoriteProductId).sort((a, b) => b[1] - a[1])[0]?.[0];
    const topProductName = inventory.find(i => i.id === topProductId)?.name || 'N/A';

    return {
      name: customer.name,
      id: customer.id,
      visitCount: customerTx.length,
      totalSpent,
      favoriteProduct: topProductName,
      lastActive: customerTx.length > 0 ? customerTx.sort((a, b) => b.date.localeCompare(a.date))[0].date : 'Never'
    };
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex bg-stone-100 p-1 rounded-xl w-fit">
          <button
            ...
            className={cn(
              ...
              reportType === 'products' ? "bg-white text-emerald-600 shadow-sm" : "text-stone-500 hover:text-stone-700"
            )}
          >
            Product Performance
          </button>
          <button
            ...
            className={cn(
              "px-6 py-2 rounded-lg text-sm font-bold transition-all",
              reportType === 'customers' ? "bg-white text-emerald-600 shadow-sm" : "text-stone-500 hover:text-stone-700"
            )}
          >
        ...
        <select
          ...
          className="bg-white border border-stone-200 rounded-xl px-4 py-2 text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all cursor-pointer"
        >
      ...
      {reportType === 'products' ? (
        <div className="space-y-6">
          ...
          {/* Product Data Table */}
          <div className="bg-white rounded-3xl border border-stone-100 shadow-sm overflow-hidden text-sm">
              ...
              <table className="w-full text-left">
                <thead className="bg-stone-50 text-stone-500 sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[10px]">Orders</th>
                    <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[10px]">Qty Sold</th>
                    <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[10px]">Revenue</th>
                    <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[10px]">Stock</th>
                ...
                <tbody className="divide-y divide-stone-100">
                  {productPerformance.map((product) => (
                    <tr key={product.id} className="hover:bg-stone-50 transition-colors">
                      ...
                      <td className="px-6 py-4">{product.salesCount}</td>
                      <td className="px-6 py-4">{product.totalQty}</td>
                      <td className="px-6 py-4 font-bold text-emerald-600">₦{product.totalRevenue.toLocaleString()}</td>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white p-8 rounded-3xl border border-stone-100 shadow-sm">
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={customerPerformance.slice(0, 8)} layout="vertical">
                    ...
                    <Tooltip 
                      ...
                      contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }}
                    />
                    <Bar dataKey="totalSpent" fill="#10b981" radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>

            <div className="bg-white p-8 rounded-3xl border border-stone-100 shadow-sm flex flex-col justify-center">
              <div className="text-center">
                <div className="w-20 h-20 bg-stone-50 rounded-3xl flex items-center justify-center mx-auto mb-6 text-emerald-600 shadow-inner">
                  <Sparkles size={40} />
                </div>
                <h3 className="text-2xl font-bold mb-2">Customer Retention</h3>
                <p className="text-stone-500 text-sm mb-8 px-8">
                  Your top {customerPerformance.slice(0, 3).length} customers account for 
                  <span className="font-bold text-emerald-600 mx-1">
                    ₦{customerPerformance.slice(0, 3).reduce((acc, c) => acc + c.totalSpent, 0).toLocaleString()}
                  </span> 
                  in revenue during this period.
                </p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-stone-50 p-4 rounded-2xl">
                    <p className="text-[10px] uppercase tracking-wider font-bold text-stone-400 mb-1">Total Customers</p>
                    <p className="text-2xl font-bold">{customers.length}</p>
                  </div>
                  <div className="bg-stone-100 p-4 rounded-2xl">
                    <p className="text-[10px] uppercase tracking-wider font-bold text-stone-400 mb-1">Repeat Clients</p>
                    <p className="text-2xl font-bold text-emerald-600">
                      {customerPerformance.filter(c => c.visitCount > 1).length}
                    </p>
                  </div>
                </div>
              </div>
          ...
          {/* Customer Performance Data Table */}
          <div className="bg-white rounded-3xl border border-stone-100 shadow-sm overflow-hidden text-sm">
            <div className="overflow-x-auto overflow-y-auto max-h-[800px]">
              <table className="w-full text-left">
                <thead className="bg-stone-50 text-stone-500 sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[10px]">Visits</th>
                    <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[10px]">Total Spends</th>
                    <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[10px]">Favorite Item</th>
                    <th className="px-6 py-4 font-semibold uppercase tracking-wider text-[10px]">Last Purchase</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-stone-100">
                  {customerPerformance.map((customer) => (
                    <tr key={customer.id} className="hover:bg-stone-50 transition-colors">
                      ...
                      <td className="px-6 py-4 font-bold text-emerald-600">₦{customer.totalSpent.toLocaleString()}</td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-stone-500">
                        {customer.lastActive !== 'Never' ? format(parseISO(customer.lastActive), 'MMM d, yyyy') : 'N/A'}
                      </td>
                    </tr>
                  ))}
                </tbody>
  ...
  const confirmLookup = async () => {
    ...
    if (user) {
      try {
        ...
        handleFirestoreError(e, OperationType.WRITE, `users/${user.id}/business_info/info`);
      }
    }
  };
...
function SettingsView({ user, business, onUpdate, onNavigate }: { user: User | null, business: BusinessInfo | null, onUpdate: () => void, onNavigate: (tab: any) => void }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      handleFirestoreError(error, OperationType.LIST, `users/${user.id}/billing_history`);
    });
    return () => unsubscribe();
  }, [user?.id]);
